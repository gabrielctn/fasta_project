...
// Recherche récursive d'une séquence/sous-séquence dans la liste chaînée de séquences
int searchBySequence(Sequences *seq, int occ, char *search) {
    ...
    // tant que le nombre d'occurence n'a pas été trouvé ou si occ=0
    if (nbSeq < occ || occ == 0) {
...
/* Recherche récursive d'une séquence à partir d'une position donnée par l'utilisateur
 * ainsi que du chromosome de la séquence supposée être trouvée */
...
// Recherche une regex donnée par l'utilisateur dans les séquences
